...
void *dragon_draw_worker(void *data){
  ...
  int startcanvas = (tdat->id/tdat->nb_thread)*(tdat->dragon_width * tdat->dragon_height);
  int endcanvas = ((tdat->id + 1)/tdat->nb_thread) * (tdat->dragon_width*tdat->dragon_height);
  int start = (tdat->id/tdat->nb_thread)*tdat->size;
  int end = ((tdat->id + 1)/tdat->nb_thread) * tdat->size;
